...
@section('content')
    ...
    <x-forms.auth-forms title="Авторизация" action="{{ route('login.handle') }}" method="POST">
        ...
        <x-slot:recRegLinks>
            <div class="space-y-3 mt-5">
                <div class="text-xxs md:text-xs">
                    <a href="{{ route('forgot') }}" class="text-white hover:text-white/70 font-bold">Забыли
                        пароль?</a>
                </div>
                <div class="text-xxs md:text-xs">
                    <a href="{{ route('reg') }}" class="text-white hover:text-white/70 font-bold">Регистрация</a>
                </div>
            </div>
        </x-slot:recRegLinks>
